...
import (
	...
	"gitlab.lan/Rightnao-site/microservices/graphql/server_ws"

	"gitlab.lan/Rightnao-site/microservices/graphql/graphiql"
...
func NewGqlServer(res interface{}, port string, corsOptions *cors.Options) *GqlServer {
	return &GqlServer{
		Schema: graphql.MustParseSchema(
			schema.GetRootSchema(),
			res,
			graphql.Tracer(CustomOpenTracingTracer{}),
			// graphql.MaxDepth(12),
...
func (g *GqlServer) Serve() error {
	...
	graphqlWSHandler := graphqlws.NewHandlerFunc(g.Schema, &relay.Handler{Schema: g.Schema})
	...
	mux.Handle("/graphqlws", graphqlWSHandler)
	mux.Handle("/graphiql", graphiql.GetGraphiql())
	mux.Handle("/libs/", http.StripPrefix("/libs", http.FileServer(http.Dir("./graphiql/libs"))))
...
func (h *httpServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// check if it application/json or application/graphql
	...
	for i, q := range req.requests {
		go func(i int, q gqlRequest) {
			h.Request = q
			//---
			ctx := middleware.GetInfo(r.Context(), r)
			//---
			res := h.Schema.Exec(ctx, q.Query, q.OpName, q.Variables)
			// FIXME expand returned hc-errors to handle a resolver returning more than one error
			responses[i] = res
			wg.Done()
		}(i, q)
	}
